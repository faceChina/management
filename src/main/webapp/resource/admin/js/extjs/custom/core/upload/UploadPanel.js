...
UploadPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		downloadPath=config.downloadUrl || getRootPath()+"/file-download";
		this.progressInfo = {
			filesTotal : 0,
			filesUploaded : 0,
			bytesTotal : 0,
...
//				url :ROOT_PATH +'/system/listAllFileAttach.do',
				url :ROOT_PATH +'/file-init',
				baseParams : config.initParams,
				autoLoad : true,
				fields : ['filePath','fileId', 'fileName', 'fileSize', 'fileType', 'fileState'],
...
//			 console.log(post_params);
			 //post_params.path = encodeURI(ROOT_PATH + '');
...
//		var ids = '';var names = '';var paths = '';
		...
		store.each(function(record) {
			...
			if (record.data.fileState != SWFUpload.FILE_STATUS.QUEUED
			&& record.data.fileState != SWFUpload.FILE_STATUS.IN_PROGRESS) {
				arrFileInfo [i] = record.get('fileId')+','+record.get('fileName')+','+record.get('filePath')+','+record.get('fileSize');
				i++;
			}
		});	
//		return ids.replace(',','');
	...
	switch (_v) {
			case SWFUpload.FILE_STATUS.QUEUED :
				returnValue = '<span id="' + record.id + '"><img id="fileId_' + record.data.fileId
						+ '" src="'+ROOT_PATH+'/images/upload/icons/delete.gif"/></span>';
				break;
			case SWFUpload.FILE_STATUS.CANCELLED :
				returnValue = '<span id="' + record.id + '"><img id="fileId_' + record.data.fileId
						+ '" src="'+ROOT_PATH+'/images/upload/icons/bin_empty.png"/></span>';
				break;
			case SWFUpload.FILE_STATUS.COMPLETE :
				returnValue = '<span id="' + record.id + '"><a href="'+downloadPath+"?fileId="+record.data.fileId+'" target="_blank"><img id="fileId_' + record.data.fileId
						+ '" src="'+ROOT_PATH+'/images/upload/icons/tick.gif"/></span>';
				break;
			default :
				alert('没有设置图表状态');
				break;
	...
	onDeleteAction : function(deleteUrl,ids){
		Ext.Ajax.request({
				url: deleteUrl,
				params: {ids : ids},
				method: 'POST',
