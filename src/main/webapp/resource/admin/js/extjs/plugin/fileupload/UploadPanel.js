...
UploadPanel = Ext.extend(Ext.Panel, {
	...
	constructor : function(config) {
		downloadPath=config.downloadUrl || __ctxPath+"/file-download";
		this.progressInfo = {
			filesTotal : 0,
			filesUploaded : 0,
			bytesTotal : 0,
		...
		var fileStore = new Ext.data.JsonStore({
				url :__ctxPath +'/sys/sysAttFile/findListByIds.do',
				baseParams : config.initParams,
				autoLoad : true,
				fields : ['filePath',
				'fileId', 
				'fileName', 
				...
				'fileState',
				'fileUrl'],
				pruneModifiedRecords : true //清除修改痕迹
		});
		this.fileList = new Ext.grid.GridPanel({
			...
			tbar : [{
				...
				handler : this.clearNotUploadList,
				scope : this
			}, {
				text : '清空列表',
				iconCls : 'db-icn-trash',
			...
			listeners : {
				...
				celldblclick : {
					fn : function(grid, rowIndex, columnIndex, e) {
						var record = grid.getSelectionModel().getSelected();
						if(record.data.fileState == SWFUpload.FILE_STATUS.COMPLETE){
							if(this.downloadUrl.indexOf('?')!=-1){
								top.location.href=this.downloadUrl+'&id='+record.data.fileId;
							}else{
								top.location.href=this.downloadUrl+'?id='+record.data.fileId;
							}
						}
				...
				render : {
					fn : function() {
						...
						var em = grid.getTopToolbar().items.get(0).el.child('em');
						...
						//
						Ext.getCmp('upload-info-panel-tbar').add(
							{xtype:'label',text:'模块允许上传文件数为 [ '+this.fileCount+' ] 个，文件大小限制为 [ '+this.fileSize+' ] 以内'},
							'->',{
								text:'帮助',
								height:25,
								iconCls:'help',
								handler:function(btn){
									alert('若出现选择文件不会自动添加到列表中，就说明您要上传的文件已经超出限制大小！');
								}
							}
						);
						Ext.getCmp('upload-info-panel-tbar').doLayout();
						//
						var settings = {
...
//			 console.log(post_params);
			 //post_params.path = encodeURI(__ctxPath + '');
//			this.swfupload.setPostParams(post_params);
	...
	remove : function(){
		...
		var selections = fileList.getSelectionModel().getSelections();
		var selCount = selections.length;
		if(selCount > 0){
			Ext.MessageBox.confirm("提示","您是否要执行删除操作?",
				function(e){
					if(e=='yes'){
						var _i = 0;
						var ids= null;
						var _fileNames = new Array();
						var _fileDiskPath = new Array();
						Ext.each(selections,function(record){
							if (record.data.fileState != SWFUpload.FILE_STATUS.QUEUED
									&& record.data.fileState != SWFUpload.FILE_STATUS.IN_PROGRESS) {
								ids = new Array(),_fileNames= new Array(),_fileDiskPath = new Array();
								ids[_i] = record.get('fileId');
								_fileNames[_i] =record.get('fileName');
								_fileDiskPath[_i] = record.get('fileDiskPath');
							}else if (record.data.fileState != SWFUpload.FILE_STATUS.CANCELLED) {
								thiz.swfupload.cancelUpload(record.get('fileId'),false);
								thiz.progressInfo.filesTotal -= 1;
								thiz.progressInfo.bytesTotal -= record.get('fileSize');
								thiz.updateProgressInfo();								
							}						
							store.remove(record);
						},this);
						if(ids != null){
							thiz.onDeleteAction(thiz.deleteUrl,ids,_fileNames,fileDiskPath);
						}
					}
				}
			);			
	...
	clearNotUploadList:function(){
		var store = this.fileList.getStore();
		var onDeleteAction = this.onDeleteAction;
		var deleteUrl = this.deleteUrl;
		if(store.getCount() > 0){
			Ext.MessageBox.confirm("提示","您是否要执行清空列表操作?",function(e){
				if(e=='yes'){
					var ids = '';
					store.each(function(record) {
						if (record.data.fileState != SWFUpload.FILE_STATUS.COMPLETE
								&& record.data.fileState != SWFUpload.FILE_STATUS.IN_PROGRESS) {
							ids+=','+record.get('fileId');
							store.remove(record);
						}
					});
				}
		...
		if(store.getCount() > 0){
			Ext.MessageBox.confirm("提示","您是否要执行清空列表操作?",function(e){
				if(e=='yes'){
					...
					if(ids != ''){
						ids = ids.replace(',','');
						thiz.onDeleteAction(thiz.deleteUrl,ids);
					}
				}
			});
		}
...
//		var ids = '';var names = '';var paths = '';
		var arrFileInfo = new Array();
		store.each(function(record) {
			//这里火狐获取到的fileState没有起反应。与SWFUpload.FILE_STATUS.QUEUED相同了，
			//这里要写个方法获取是不是IE 。
			if (record.data.fileState != SWFUpload.FILE_STATUS.QUEUED
			&& record.data.fileState != SWFUpload.FILE_STATUS.IN_PROGRESS) {
				arrFileInfo [i] = record.get('fileId')+','+record.get('fileName')+','+record.get('filePath')+','+record.get('fileSize')+','+record.get('fileUrl');
				i++;
			}
		});	
//		return ids.replace(',','');
	...
	switch (_v) {
			case SWFUpload.FILE_STATUS.QUEUED :
				returnValue = '<span id="' + record.id + '"><img id="fileId_' + record.data.fileId
						+ '" src="'+__ctxPath+'/images/upload/icons/delete.gif"/></span>';
				break;
			case SWFUpload.FILE_STATUS.CANCELLED :
				returnValue = '<span id="' + record.id + '"><img id="fileId_' + record.data.fileId
						+ '" src="'+__ctxPath+'/images/upload/icons/bin_empty.png"/></span>';
				break;
			case SWFUpload.FILE_STATUS.COMPLETE :
				returnValue = '<span id="' + record.id + '"><a href="'+downloadPath+"?fileId="+record.data.fileId+'" target="_blank"><img id="fileId_' + record.data.fileId
						+ '" src="'+__ctxPath+'/images/upload/icons/tick.gif"/></span>';
				break;
			default :
				alert('没有设置图表状态');
				break;
	...
	onDeleteAction : function(deleteUrl,ids,fileNames,fileDiskPath){
		Ext.Ajax.request({
				url: deleteUrl,
				params: {
					ids : ids,
	...
	onFileQueued : function(file) {
		...
		var fileList = thiz.fileList;
		var store = fileList.getStore();
		if(store.getCount() >= thiz.fileCount){
			Ext.MessageBox.alert('提示','您只允许上传'+thiz.fileCount+'个附件！');
			thiz.swfupload.cancelUpload(file.id,false);
			return;
		}				
		store.add(new UploadPanel.FileRecord({
			fileId : file.id,
			fileName : file.name,
			fileSize : file.size,
			fileType : file.type,
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (data.success) {
			...
			record.set('fileId',data.fileId);
			record.set('filePath',data.filePath);
			record.set('fileName',data.fileName);
			record.set('fileUrl',data.fileUrl)
			record.commit();
		}
