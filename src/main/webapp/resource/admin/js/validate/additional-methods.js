...
jQuery.validator.addMethod("newvaluenmber", function(value, element) {
	return this.optional(element) || /^[1-9]{1}\d*$/.test(value);
}, function(){
	alert('请填写正确的数量！');
});

jQuery.validator.addMethod("itemVali", function itemVali(value, element){
	 var colorlen = $('.new_spys').find('input[type="checkbox"]:checked').length;
	 var sizelen = $('.new_shcm').find('input[type="checkbox"]:checked').length;
	 var stocklen = 0;
	 var flag = false;
	 $('.shuliang').each(function(){
		 var stock = 0;
		 $(this).find('input').each(function(){
			 $(this).val()>0?stock++:stock;
		 });
		 if(stock==0){
			 stocklen++;
		 }
	 });
	 (colorlen>0 && sizelen>0 && stocklen==0)?flag = true:flag = false;
	 return flag;
	}, function(){
		alert('请填写正确的数量！');
	});
